...
El flujo magnético ($\phi$) se define, en una superficie, como el número de líneas de inducción que la atraviesan. Este número número de líneas por superficie perpedincular a las mismas indica la intensidad del campo, es decir:

\begin{equation}\label{INDUCC_FLUJO1}
	\phi = \vec{B} \cdot \vec{S} \Longrightarrow \phi = B \cdot S \cdot \cos \varphi 
\end{equation}

El flujo magnético se mide en Weber (\unit{\weber}).
...
La Ley de Faraday--Lenz establece que cuando el \textbf{flujo magnético} que atraviesa un circuito varía, aparece una \textbf{fuerza electromotriz} (fem) inducida. Su valor viene dado por:

\[
\varepsilon = -\frac{d\phi}{dt}
\]

El signo \textbf{negativo} indica que la corriente inducida \textbf{siempre se opone} al cambio de flujo que la produce (sentido de Lenz). La fem \(\varepsilon\) es la energía por unidad de carga que surge en el circuito debido a esa variación de flujo y se mide en voltios (\unit{\volt}). \textbf{Ejemplo:} Si un imán se acerca a una espira, el flujo magnético aumenta y aparece una corriente cuyo campo se opone a ese acercamiento. Si el imán se aleja, el flujo disminuye y la corriente inducida cambia de sentido para intentar mantener el flujo anterior.

\clearpage

\begin{mybox}{Ejercicio Ejemplo PAU}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[scale=0.2]{INDUCC_EJEMPLO1}
		\caption{}
	...
	Sustituyendo los datos que ya tenemos:

	\begin{equation}
		\phi (t) = B \frac{4t^2}{2} = 2Bt^2 
	\end{equation}

	Con esto, podemos calcular la fem inducida:

	\begin{equation}
		\varepsilon = -\frac{d\phi}{dt} = -4Bt
	\end{equation}
