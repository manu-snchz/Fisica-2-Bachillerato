...
Si un hilo que transporta una corriente eléctrica entra en contacto con un campo magnético, sufrirá una fuerza magnética que podemos deducir. La intensidad, por definición se define como: $I = \frac{dq}{dt}$, y sustituyendo en la ecuación de la Ley de Lorentz obtenemos:

\begin{equation}
	d\vec{F_B} = dq \cdot \vec{v} \times \vec{B} \Longrightarrow \vec{F_B} = I \cdot dt \cdot \vec{\ell} \times \vec{B}
\end{equation}

\begin{equation}
	\vec{F} = I\vec{\ell} \times \vec{B} \Longrightarrow \vec{F} = I \cdot \ell \cdot B \cdot \sin\alpha
\end{equation}

En esta fórmula, la dirección y sentido de $\vec{\ell}$ coinciden con el de la corriente, y su módulo con la longitud del hilo. 

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.7]{MAGN_FUERZAHILO}
	\caption{Fuerza sobre un hilo de corriente}
